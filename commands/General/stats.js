...
module.exports = class extends Command {
	...
	async run(message) {
		...
		let memUsage = (memory || process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2);
		let uptime = Duration.toNow(Date.now() - (process.uptime() * 1000));
		let userSize = (users || this.client.users.size).toLocaleString()
		let guideSize = (guilds || this.client.guilds.size).toLocaleString();
		let channelSize = (channels || this.client.channels.size).toLocaleString();
